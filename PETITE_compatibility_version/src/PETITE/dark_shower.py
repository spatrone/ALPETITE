...
class DarkShower(Shower):
    ...
    def set_dark_samples(self):
        ...
        for process in diff_xsection_options.keys():
            self._loaded_dark_samples[process]= self.load_dark_sample(self._dict_dir, process)
    ...
    def draw_dark_sample(self,Einc,LU_Key=-1,process="DarkBrem",VB=False):
        ...
        if sample_found is False:
            sys.stdout.write(f"\rNo Sample Found: Process={process}, Einc={Einc}, LU_Key={LU_Key}")
            sys.stdout.flush()
        if VB:
            return np.concatenate([list(x), [sampcount]])
        else:
            return np.array(x), f_integrand(x)
        #SAMMod_END
    ...
    def produce_bsm_particle(self, p_original, process, weight=None, VB=False):
        ...
        if process == "DarkAnn" and self.bound_electron and E0 <= self._resonant_annihilation_energy:
            ...
            #SAMMod_START
            while True:
                sample_event, diff_x_sec = self.draw_dark_sample(E0, process=process, VB=VB)
                if process!="DarkBrem":
                    break
                else:
                    if len(sample_event)!=3: 
                        print("Point Not Valid! Resampling...")
                    else:
                        break
